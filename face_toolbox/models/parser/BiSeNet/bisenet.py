...
def upsampling(x, shape, interpolation="nearest"):
    if interpolation == "nearest":
        return Lambda(lambda t: tf.compat.v1.image.resize(t, shape, method=tf.compat.v1.image.ResizeMethod.NEAREST_NEIGHBOR))(x)
    elif interpolation == "bilinear":
        return Lambda(lambda t: tf.compat.v1.image.resize(t, shape, method=tf.compat.v1.image.ResizeMethod.BILINEAR))(x)
